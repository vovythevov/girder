...
class HashStateTestCase(base.TestCase):
    ...
    def setUp(self):
        ...
        # Arbitrary number to be able to split the python zen in a least a few chunks
        chunkSize = len(testChunk) / 6
        self.Chunks = [testChunk[i:i+chunkSize] for i in range(0, len(testChunk), chunkSize)]
    ...
    def testSimpleHashing(self):
        """
        Test all the base algorithms against the python's hashlib
        """
